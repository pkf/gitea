...
func TransferOwnership(doer *User, newOwnerName string, repo *Repository) error {
    ...
    // 如果新owner是组织，检查原作者是否组织成员，不是不允许转入
    if newOwner.IsOrganization() {
        if has, err = newOwner.IsOrgMember(doer.ID); err != nil {
            return errors.New("检查是否拥有产品组权限失败")
        } else if !has {
            return errors.New("非该产品组成员不允许转入仓库")
        }
    }
