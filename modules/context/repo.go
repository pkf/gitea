...
func repoAssignment(ctx *Context, repo *models.Repository) {
    ...
    if ctx.Repo.AccessMode == models.AccessModeNone && repo.Owner.IsOrganization() {
        // 如果没有权限，但是组织成员，可以访问部分库
        switch repo.Name {
        case "Document":
            isMember, err := repo.Owner.IsOrgMember(ctx.User.ID)
            if err == nil || isMember {
        ...
        case "PMDocument":
            pmteam, err := repo.Owner.GetTeam("PM")
            if err == nil {
                if pmteam.IsMember(ctx.User.ID) {
                    ctx.Repo.AccessMode = models.AccessModeWrite
                }
            }
        default:
