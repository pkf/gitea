...
func showOrgProfile(ctx *context.Context) {
    ...
    var pmDocRepo,docRepo *models.Repository
    for _, r := range repos {
        switch r.Name{
        case "Document":
            docRepo = r
    ...
    ctx.Data["PMDocRepo"] = pmDocRepo
    ctx.Data["HasDocRepo"] = docRepo!=nil || pmDocRepo!=nil
    ctx.Data["Repos"] = tmpRepos
    ctx.Data["Page"] = paginater.New(int(count), setting.UI.User.RepoPagingNum, page, 5)
    
    if docRepo != nil {
        if docRepo.OwnerName == ""{
            docRepo.OwnerName = org.Name
        }
        data, ok := getFileContentFromDefaultBranch(docRepo, "README.md")
        if ok && base.IsTextFile(data){
            treeLink := docRepo.Link() + "/src/branch/" + docRepo.DefaultBranch +"/"
            ctx.Data["IndexFileContent"] = string(markup.Render("README.md", data, path.Dir(treeLink), map[string]string{}))
        }
    }
...
func getFileContentFromDefaultBranch(repo *models.Repository, filename string) ([]byte, bool) {
    if repo == nil || repo.IsBare {
        //如果仓库为空仓库
        return nil, false
    }
    
    gitRepo, err := git.OpenRepository(models.RepoPath(repo.OwnerName, repo.Name))
    if err != nil {
        log.Error(5, "%s: %s %s", repo.OwnerName, repo.Name,err)
        return nil, false
    }
    ...
    if entry.Blob().Size() >= setting.UI.MaxDisplayFileSize {
        log.Error(5, "%s: %s %d", repo.OwnerName, repo.Name,entry.Blob().Size())
        return nil, false
    }
